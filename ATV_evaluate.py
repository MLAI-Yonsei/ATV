...
class Retrieve_Evaluator:
            ...
            if self.args.single_layer_mode:
                ...
                for split in ["train", "valid", self.args.dataset_split]:
                    if split in dataset and hasattr(dataset[split], "raw_data"):
                        ...
                        dataset[split].raw_data = dataset[split].raw_data.sample(n=n_samples, random_state=self.args.seed)
    ...
    def forward_kwargs(self):
        ...
        if getattr(self.args, "logits_to_keep", False):
    ...
    def retrieve_and_intervene_all_layers(self, dataset_split, template=None, nshots=None):
        ...
        for batch_start in tqdm(range(0, dataset_len, batch_size)):
            ...
            end_time = torch.cuda.Event(enable_timing=True)
            start_time.record()
            clean_logits = self.model(**clean_inputs, **self.forward_kwargs()).logits  # [B, seq_or_1, vocab]
            end_time.record()
            torch.cuda.synchronize()
            ...
            with TraceDict(self.model, layers=self.model_config["layer_hook_names"], edit_output=intervention_fn, retain_output=False):
                intervene_logits = self.model(**clean_inputs, **self.forward_kwargs()).logits  # [B, seq_or_1, vocab]
            end_time.record()
            torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trained_model_path", type=str, required=True, help="Path to the trained GPT-2 and projection layer model")
